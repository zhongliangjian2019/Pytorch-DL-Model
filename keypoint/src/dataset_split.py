...
def data_split(data_dir: str, train_ratio: float = 0.9, val_ratio: float = 0.1):
    """分割任务数据集划分"""
    # 提取具有标注的图像文件
    image_files = [name for name in os.listdir(os.path.join(data_dir, "images")) if name.split('.')[-1] in ['jpg', 'png', 'bmp', 'tif']]
    mask_files = [name.split('.')[0] for name in os.listdir(os.path.join(data_dir, "labels")) if name.endswith('txt')]
...
if __name__ == '__main__':
    data_dir = r"Y:\model_train\keypoint_model\data\qrcode_keypoints"
    data_split(data_dir)
