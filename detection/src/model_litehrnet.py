...
from model_module import conv1x1, DSConvBNAct, DWConvBNAct, ConvBNAct, channel_shuffle
    ...
    def forward(self, feats):
        ...
        for i, feat in enumerate(feats):
            if i == len(feats)-1:
                ...
                import math
                kh = math.ceil(feat.size()[2] / pool_size[0])
                kw = math.ceil(feat.size()[3] / pool_size[1])
                cr_weight.append(F.avg_pool2d(feat[:, ch_r[i]:], kernel_size=(kh, kw), stride=(kh, kw)))
...
if __name__ == "__main__":
    ...
    model.eval()
    # torch.save(model.state_dict(), "lite_hrnet.pth")
