...
def ShowImage(image, winname: str = 'test', winsize: tuple = (1000, 1000), wait: int = 0):
    """显示单张图像"""
    cv2.namedWindow(winname, cv2.WINDOW_KEEPRATIO)
    cv2.resizeWindow(winname, winsize[0], winsize[1])
...
def ShowImages(image_list: object, winname: str = 'test', winsize: tuple = (1000, 1000), wait: int = 0) -> object:
    """显示多张图像"""
    if not isinstance(image_list, list):
        show_image = image_list
        ShowImage(show_image, winname, winsize, wait)
    else:
        show_list = []
        # 统一图像为BGR
        ...
        new_size = (len(image_list) * winsize[0], winsize[1])
        ShowImage(show_image, winname, new_size, wait)
...
def PreProcess(in_image, size=(49, 49)):
    """数据预处理"""
    # 图像缩放到输入大小
    if (in_image.shape[0] != size[0]) or (in_image.shape[1] != size[1]):
        out_image = cv2.resize(in_image, dsize=size)
    else:
        out_image = in_image.copy()

    crop_images = CropImage5(out_image)

    out_images = [out_image]
    for image in crop_images:
        out_images.append(cv2.resize(image, size))

    out_image = cv2.merge(out_images)
...
def softmax(input, axis=1):
    ...
    :return: output
    """
    assert input.ndim == 2, "error info: input dim mismatching, expect input shape is (N,C)"
    output = np.exp(input)
    output = output / np.reshape(output.sum(axis=axis), (-1, 1))
    return output
